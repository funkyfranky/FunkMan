...
class FunkBot(commands.Bot):
    ...
    async def on_ready(self):
        ...
        try:
            ...
            log.critical(f"ERROR: Could not get channel with ID={self.channelID}")
            quit()

        # Info that bot is online and ready.
        log.info("Connected as {0.name} [ID: {0.id}]".format(self.user))
        ...
        if self.debugLevel >= 20 and self.debug:
    ...
    async def on_disconnect(self):
        ...
        try:
            ...
            log.error(f"ERROR: Could not get channel with ID={self.channelID}")
    ...
    def Start(self, Threaded=False):
        ...
        """
        if Threaded:
            log.info("Starting threaded discord bot!")
            discordThread = threading.Thread(target=self.Start, args=(False,))
            discordThread.start()
        else:
            log.info(f"Starting Bot Client with Token {self.token[0:5]}...")
            self.run(self.token)
    ...
    def SendText(self, Text: str, ChannelID: int):
        ...
        try:
            ...
            log.error(f"ERROR: Could not send text! {Text}")
    ...
    def SendLSOEmbed(self, result, ChannelID: int):
        ...
        # Create Embed
        embed = discord.Embed(
            title="LSO Grade", description=f"Result for {player} at carrier {carriername} [{carriertype}]", color=color
        )
